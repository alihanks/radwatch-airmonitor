...
#color palette
col_pal=open('../etc/colr/col_scheme.dat').readlines();col_pal=[el.replace('\n','') for el in col_pal];
...
for el in col_comp.rois:
    ...
    lst_peak=el.peak[1];lst_peak_height=spec[super_tmp];
...
if( plot_most_recent_spec==True ):
    ...
    xlim([0,3000]);xlabel('Energy (keV)');ylabel('Counts');title('Past Hour\'s Gamma Spectrum');
...
gcf().add_subplot(axarr[0]);gcf().add_subplot(axarr[0]);
...
for x in range(0,len(col.rois)):
    ...
    axarr[1].errorbar(timestamps,roi_array[:,x,0],yerr=roi_array[:,x,1],marker='+',label=col.rois[x].isotope,color=col_pal[4+x]);
    axarr[2].plot(timestamps,weather[:,6],color=col_pal[2]);
    bar_ax0.plot(timestamps,weather[:,2],color=col_pal[3]);

out_file=open('weather.csv','w+');
out_file_bq=open('weather_bq.csv','w+');
...
for x in range(len(timestamps)-15*24,len(timestamps)):
    ...
    for y in range(0,len(roi_array[x,:,0])):
        ...
        out_str_bq+=","+str((roi_array[x,y,0]-roi_array[mins[y],y,0])/eff)+","+str(np.sqrt(roi_array[x,y,1]**2+roi_array[mins[y],y,1]**2)/eff) ;
        
        out_str+=","+str((roi_array[x,y,0]-roi_array[mins[y],y,0]))+","+str(np.sqrt(roi_array[x,y,1]**2+roi_array[mins[y],y,1]**2)) ;
